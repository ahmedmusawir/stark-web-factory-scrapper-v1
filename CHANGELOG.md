...
- **bim000 test amendments (AC-71), listed as made:**
  ...
  - (c) `tests/test_crawler.py::_result()` stub — optional `html=None` parameter; attribute set only when provided, absent by default. `FakeCrawler.arun` additionally raises when handed an Exception instance (test helper only). (chunk 3)
